...
char GSM::SendATCmdPrintResp(char const *AT_cmd_string,
                uint16_t start_comm_tmout, uint16_t max_interchar_tmout,
                ...
                byte no_of_attempts)
{
  ...
  for (i = 0; i < no_of_attempts; i++) {
    ...
    mySerial.println(AT_cmd_string);
	byte j =0;
	while (AT_cmd_string[j]){
	Serial.print(byte(AT_cmd_string[j]));
	Serial.print("+");
	j=j+1;
...
//Serial.printl(comm_buf);	
      ...
      // nothing was received
      // --------------------
      Serial.print("\rNothing resived\r");
    }
...
char GSM::InitGPRS(void)
{
  ...
  if (AT_RESP_OK == SendATCmdWaitResp("AT+CMEE=1", 1000, 1000, "OK", 10)) {
	if (AT_RESP_OK == SendATCmdWaitResp("AT+CGATT=1", 1000, 1000, "OK", 10)){
		if (AT_RESP_OK == SendATCmdWaitResp("AT+CSTT=\"internet.beeline.ru\",\"beeline\",\"beeline\"", 10000, 1000, "OK", 1)){
			if (AT_RESP_OK == SendATCmdWaitResp("AT+CIICR", 1000, 1000, "OK", 10)){
				ret_val = 1;
			}else{ret_val = -8;};
		}else{ret_val = -7;};
	}else{ret_val = -6;};
...
host_str:   pointer to the host address string
port_str:   pointer to the port number
message_str:  pointer to the message string
...
an example of usage:
        GSM gsm;
        gsm.SendTCP("host_name","port", "text");
**********************************************************/
char GSM::SendTCP(char *host_str, char *port_str, char *message_str) 
{
  char ret_val = -1;
  byte i;
  ...
  mySerial.write("AT+CIPSTART=\"TCP\",\"");
    mySerial.write(host_str);  
    mySerial.write("\",\"");
    mySerial.write(port_str);  
    mySerial.write("\"\r");
	Serial.print("\r\nCIPSTART=\"TCP\",\"");
	...
	if (RX_FINISHED_STR_RECV == WaitResp(10000, 500, "OK")) {
	ret_val = 0;
	}else{
	ret_val = -3;
...
/*  ret_val = 1;
 ...
 Serial.println("---");
  ...
  // try to send MSG 3 times in case there is some problem
  for (i = 0; i < 10; i++) {
    // send  AT+CMGS="number_str"
    mySerial.write("AT+CIPSEND=");
    mySerial.print(strlen(message_str));  
    mySerial.write("\r");
	Serial.write("SEND:");
	Serial.print(strlen(message_str));
	Serial.write("Bt\n");
     if (RX_FINISHED_STR_RECV == WaitResp(5000, 1000, ">")) {
      // send  text
      mySerial.write(message_str); 
  if (RX_FINISHED_STR_RECV == WaitResp(7000, 5000, "SEND OK")) {
        ret_val = 1;
        ...
        ret_val = -3;
	Serial.write("No > simbol recived \r\n");
	Serial.print((char *)(comm_buf));
	delay(2000);
  ...
  mySerial.write("\r");
  if (RX_FINISHED_STR_RECV == WaitResp(7000, 5000, "OK")) {
  Serial.write("CIPSHUT OK \r\n");
  }else{
